...
run_pindel(){
    ...
    
    {
        ...
        cat ${pindel_dir}/${case_ID}.D.head ${pindel_dir}/${case_ID}.SI.head > ${pindel_dir}/all.head

        #step 3: run the provided perl script with an updated configuration file
        cd ${pindel_dir}
        perl ${somatic_indelfilter_path}/somatic_indelfilter.pl ${somatic_indelfilter_path}/somatic.indel.filter.config
        mv indel.filter.output ./${1}.indel.filter.NorTum.20.output
